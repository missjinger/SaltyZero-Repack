        ...
        bool Load(char const* fn, SqlDbc* sql)
        {
            ...
            if (sql)
            {
                ...
                if (result)
                {
                    ...
                    {
                        fields = result->Fetch();
                        sqlHighestIndex = fields[sql->sqlIndexPos].GetValue<uint32>();
                    }
            ...
            if (result)
            {
                if (indexTable.asT)
                {
                    ...
                    do
                    {
                        ...
                        if (sql->indexPos >= 0)
                        {
                            uint32 id = fields[sql->sqlIndexPos].GetValue<uint32>();
                        ...
                        for (; columnNumber < sql->formatString->size(); ++columnNumber)
                        {
                            ...
                            else if ((*sql->formatString)[columnNumber] == FT_SQL_PRESENT)
                            {
                                ...
                                {
                                    case FT_FLOAT:
                                        *reinterpret_cast<float*>(&sqlDataTable[offset]) = fields[sqlColumnNumber].GetValue<float>();
                                        offset += 4;
                                        break;
                                    case FT_IND:
                                    case FT_INT:
                                        *reinterpret_cast<uint32*>(&sqlDataTable[offset]) = fields[sqlColumnNumber].GetValue<uint32>();
                                        offset += 4;
                                        break;
                                    case FT_BYTE:
                                        *reinterpret_cast<uint8*>(&sqlDataTable[offset]) = fields[sqlColumnNumber].GetValue<uint8>();
                                        offset += 1;
                                        break;
                                    case FT_STRING:
